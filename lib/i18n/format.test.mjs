...
test("formatSessionTimestamp uses relative time within 3 days, date beyond", () => {
  ...
  // 1 day ago — still relative
  assert.equal(formatSessionTimestamp(new Date(2026, 0, 2, 8, 0, 0), "en", now), "1 day ago");
  // 3 days ago — still relative
  assert.equal(formatSessionTimestamp(new Date(2025, 11, 31, 8, 0, 0), "en", now), "3 days ago");
  // 4 days ago — absolute date (year/month/day only, no time)
  const older = formatSessionTimestamp(new Date(2025, 11, 30, 8, 9, 7), "en", now);
  assert.match(older, /2025/);
  assert.doesNotMatch(older, /day ago/);
  // Chinese fallback for older
  const zhOlder = formatSessionTimestamp(new Date(2025, 11, 30, 5, 4, 3), "zh-CN", now);
  assert.match(zhOlder, /2025/);
  assert.doesNotMatch(zhOlder, /天前/);
});
