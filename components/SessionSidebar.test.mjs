...
test("formats session timestamps with the active locale", () => {
  assert.match(source, /import \{ calendarDaysAgo, formatSessionTimestamp, formatDayLabel \} from "@\/lib\/i18n\/format"/);
  assert.match(sessionItemSource, /const \{ locale, t \} = useI18n\(\)/);
  assert.match(sessionItemSource, /formatSessionTimestamp\(session\.modified, locale\)/);
});
...
test("lifecycle refreshes bypass the cache while cross-window polling reuses it", () => {
  ...
  // lifecycle refreshes.
  assert.match(source, /sessionRefreshDone/);
